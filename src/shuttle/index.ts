...
function fillL<T>(arr: T[], fillValue: T, multiples: number = 8) {
  if (arr.length % multiples !== 0)
    arr.unshift(
      ...new Array(multiples - (arr.length % multiples)).fill(fillValue)
    );
  return arr;
}
...
function booleansToUint(b: boolean[]) {
  ...
  return b
    .reverse()
    .map((v, i) => BigInt(+v) * bigint2 ** BigInt(i))
...
function packNoflagUint(n: number | bigint) {
  const booleans = fillL(uintToBooleans(n, 1), false, 7).reverse(),
    l = booleans.length;
  let i = 0;
...
function packNumber(n: number | bigint) {
  ...
  const flag =
    (typeof n === "bigint"
      ...
      : Number.isInteger(n)
      ? flags_INT
      : flags_FLOAT) + (negative ? 1 : 0);
  if (flag < flags_FLOAT) return new Uint8Array([flag, ...packNoflagUint(n)]);
  const decimalUint = booleansToUint(
    fillR(
      [...((n as number) % 1).toString(2).substring(2)].map((i) => i === "1"),
      false
    )
  );
  return new Uint8Array([
    flag,
    ...packNoflagUint(Math.floor(n as number)),
    ...packNoflagUint(decimalUint),
  ]);
}
...
function packString(s: string) {
  ...
  if (/^-?[0-9]+$/.test(s)) {
    const n = BigInt(s),
      flag = s.startsWith("-") ? flags_STR_NUMR_ : flags_STR_NUMR;
    if (n.toString() === s)
      return new Uint8Array([flag, ...packNumber(n).slice(1)]);
  }
  if (/^[0-9a-f]+$/.test(s)) {
    const n = hexToBigint(s);
    if (n.toString(16) === s)
      return new Uint8Array([flags_STR_HEX_LW, ...packNumber(n).slice(1)]);
  }
  if (/^[0-9A-F]+$/.test(s)) {
    const n = hexToBigint(s);
    if (n.toString(16).toUpperCase() === s)
      return new Uint8Array([flags_STR_HEX_UP, ...packNumber(n).slice(1)]);
  }
...
function unpackString(bytes: Uint8Array, p = new BufferPointer()) {
  ...
  switch (flag) {
    case flags_STR_EMPTY:
      return "";
    case flags_STR_NUMR:
      return unpackNumber(bytes, p, flags_BIGINT).toString();
    case flags_STR_NUMR_:
      return unpackNumber(bytes, p, flags_BIGINT_).toString();
    case flags_STR_HEX_LW:
      return unpackNumber(bytes, p, flags_BIGINT).toString(16);
    case flags_STR_HEX_UP:
      return unpackNumber(bytes, p, flags_BIGINT).toString(16).toUpperCase();
    case flags_STR_BIN:
      return unpackNumber(bytes, p, flags_BIGINT).toString(2);
    case flags_STR_BIN_:
      return unpackNumber(bytes, p, flags_BIGINT_).toString(2);
    default:
      const stringLength = unpackNoflagUint(bytes, p);
      return new TextDecoder().decode(
        bytes.slice(p.pos, p.walked(stringLength))
      );
  }
}
...
function packSpecial(b: boolean | null | undefined) {
  switch (b) {
    case true:
      return new Uint8Array([flags_TRUE]);
    case false:
      return new Uint8Array([flags_FALSE]);
    case null:
      return new Uint8Array([flags_NULL]);
    default:
      return new Uint8Array([flags_UNDF]);
  }
}
...
function packArray(value: any[] | Set<any> | Map<any, any>) {
  ...
  return new Uint8Array([
    flag,
    ...(value as any[]).map((v) => [..._packData(v)]).flat(),
    flags_END,
  ]);
}
...
function unpackArray(
  ...
  if (flag === flags_MAP)
    return new Map(
      arr
        .map((v, i, a) => (i % 2 === 0 ? [v, a[i + 1]] : undefined))
        .filter((i) => i !== undefined) as [any, any][]
    );
...
function packObject(value: any) {
  if (value instanceof Date) {
    ...
    return new Uint8Array([
      t < 0 ? flags_DATE_ : flags_DATE,
      ...packNoflagUint(Math.abs(t)),
    ]);
  }
  return new Uint8Array([
    flags_OBJ,
...
function unpackObject(bytes: Uint8Array, p = new BufferPointer()): object {
  ...
  while (bytes[p.pos] !== flags_END) {
    const k = _unpackData(bytes, p) as string,
      v = _unpackData(bytes, p);
    obj[k] = v;
  }
  p.walk();
  return obj;
...
function packBuffer(bytes: Uint8Array | Uint16Array | Uint32Array) {
  ...
  return new Uint8Array([
    flag,
    ...packNoflagUint(bytes.length),
    ...(bytePerElement === 1 ? bytes : convertUintArray(bytes, 8)),
  ]);
}

function unpackBuffer(bytes: Uint8Array, p = new BufferPointer()) {
  const flag = bytes[p.walk()];
  const bytePerElement = [0, flags_BUF8, flags_BUF16, 0, flags_BUF32].indexOf(
    flag
  ) as 1 | 2 | 4;
...
function _unpackData(value: Uint8Array | number[], p = new BufferPointer()) {
  ...
  switch (Math.floor(flag / 32)) {
    case 0:
      return unpackSpecial(value, p); // < 32
    case 1:
      return unpackNumber(value, p); // < 64
    case 2:
      return unpackString(value, p); // < 96
    case 3:
      return unpackArray(value, p); // < 128
    case 4:
      return unpackObject(value, p); // < 160
    case 5:
      return unpackBuffer(value, p); // < 192
    default:
      throwInvalidFlag(flag);
  }
}
...
function encryptBuffer(array: Uint8Array | number[], ...salts: number[]) {
  if (salts.length > 1)
    return encryptBuffer(encryptBuffer(array, salts[0]), ...salts.slice(1));
  if (salts.length === 0)
...
function decryptBuffer(array: Uint8Array | number[], ...salts: number[]) {
  if (salts.length > 1)
    return decryptBuffer(decryptBuffer(array, ...salts.slice(1)), salts[0]);
  if (salts.length === 0)
    return array instanceof Uint8Array ? array : new Uint8Array(array);
  const shuffledIndexes = new Random(salts[0]).shuffle(
    new Array(array.length).fill(0).map((v, i) => i)
...
function hashData<T>(
  ...
  if (typeof array !== "string")
    return hashData(Shuttle.load(array), algorithm);
